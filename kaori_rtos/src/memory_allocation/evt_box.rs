...
static PUBLIC_ALLOCATOR: SharedAllocator = SharedAllocator::new();

impl <T>EvtBox<T>{
    pub(crate) fn new(element: T) -> EvtBox<T>{
        unsafe{
            let inner = PUBLIC_ALLOCATOR.allocate(size_of::<T>()).unwrap();
...
impl<T> Drop for EvtBox<T>{
    fn drop(&mut self){
        unsafe{            
            ...
            PUBLIC_ALLOCATOR.free(self.inner as *mut u8).unwrap();
        }
    }
}
